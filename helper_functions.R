...
m2df <- function(mymatrix, row_column_name = "Row_variable"){
  ...
  mydf[, row_column_name] <- rownames(mydf)
  rownames(mydf) <- NULL
  mydf <- mydf[,c(row_column_name,cur_names)]
  return(mydf)
}
...
# STOP USING THIS, FIND MYSELF NOT TRUSTING
...
generate_correlation_network <- function(cor_matrix, p_matrix = NULL, p_value_threshold = 0.05, cor_threshold = 0.5,
  ...
  if (!is.null(p_matrix)){
    ...
    if (!is.null(relabeller_function)){
      ...
      colnames(cor_pval.m) <- lapply(colnames(cor.m), relabeller_function)
      rownames(cor_pval.m) <- lapply(rownames(cor.m), relabeller_function)
    }
  }
